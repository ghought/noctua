...
const TITLE_TOOL = {
  name: 'record_dream_title',
  description: 'Record a short title for a dream journal entry.',
  input_schema: {
    type: 'object',
    properties: {
      title: {
        type: 'string',
        description: 'Short, evocative title for this dream. 3-7 words, poetic, not a full sentence.',
      },
    },
    required: ['title'],
  },
...
app.post('/api/title', async (req, res) => {
  ...
  if (!dream?.body) {
    return res.status(400).json({ error: 'Missing dream body.' });
  }
  ...
  try {
    ...
    const message = await client.messages.create({
      model: 'claude-opus-4-7',
      max_tokens: 160,
      system: `You title private dream journal entries. Return only a short title through the tool. Be evocative and specific, but restrained. No analysis, no diagnosis, no spoilers, no full sentences.`,
      tools: [TITLE_TOOL],
      tool_choice: { type: 'tool', name: 'record_dream_title' },
      messages: [{
        role: 'user',
        content: `Dream entry:\n\n${dream.body}${moods}\n\nCreate one title. Use 3-7 words. Do not include quotation marks.`,
      }],
    });

    const toolBlock = message.content.find(b => b.type === 'tool_use');
    if (!toolBlock || toolBlock.type !== 'tool_use') {
      throw new Error('Model did not return a structured title.');
    }
...
<body>
  ...
  <p><strong>Your dream entries are stored locally on your device.</strong> We do not upload, sync, or store your dream text, moods, or titles on any server unless you explicitly use a feature that requires it (such as AI titling or interpretation).</p>
  <p>When you request an AI title or interpretation, the text of the relevant dream entry is transmitted to our server at <code>noctua.up.railway.app</code> and then to Anthropic's Claude API to generate the requested result. We do not store this text after the response is returned. We do not use your dream content to train any AI model.</p>
  <p>We collect:</p>
  <ul>
    <li>Dream text sent for AI titling or interpretation (not stored after response)</li>
  ...
  <h2>VI · AI Titling &amp; Interpretation</h2>
  <p>Titles and interpretations are generated by Anthropic's Claude API. Your dream text is sent to Anthropic solely to generate the requested title or interpretation. Anthropic's use of API data is governed by their <a href="https://www.anthropic.com/legal/privacy" target="_blank">privacy policy</a> and API usage policy — Anthropic does not use API inputs to train models by default.</p>
  ...
  <ul>
    <li><strong>Anthropic Claude API</strong> — AI titling and interpretation. <a href="https://www.anthropic.com/legal/privacy">Privacy policy</a></li>
